...
function showSecurityPreferencesPanel(chromeWindow) {
  ...
  const SECURITY_PREFERENFES_URI = 'chrome://torbutton/content/preferences.xhtml';

  tabBrowser.tabs.some(function (tab) {
      // If the security prefs tab is opened, send the user to it
      if (tab.browser.currentURI.spec === SECURITY_PREFERENFES_URI) {
          settingsTab = tab;
          return true;
      }
      return false;
  ...
  if (settingsTab === null) {
      ...
      tabBrowser.addTab(SECURITY_PREFERENFES_URI, {
          'selected': true,
